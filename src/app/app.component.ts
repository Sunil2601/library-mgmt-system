...
export class AppComponent {
  ...
  onAddBookToCartEventEmmission(bookEvent:Book){
    let cartObj={book:bookEvent,quantity:1};

    // checking if product is already in th cart
    let idx=-1;
    for(let i=0;i<this.cart?.length;i++){
      if(this.cart[i].book===cartObj.book){
        idx=i;
        break;
      }
    }
    ...
    if(idx==-1){
      ...
      this.total+=Math.floor(cartObj.book.price.value);
    }
    else alert("Product already in cart");
  }
  reduceQuantity(bookTitle:string){
    
    let idx=this.cart.findIndex(obj=>obj.book.title==bookTitle)
    if(idx>=0){
      this.cart[idx].quantity-=1;
      this.total-=Math.floor(this.cart[idx].book.price.value);
  ...
  increaseQuantity(bookTitle:string){
    let idx=this.cart.findIndex(obj=>obj.book.title==bookTitle)
    if(idx>=0){
      this.cart[idx].quantity+=1;
      this.total+=Math.floor(this.cart[idx].book.price.value);       
  ...
  removeBook(bookTitle:string){
    ...
    let idx=this.cart.findIndex(obj=>obj.book.title==bookTitle)
    if(idx>=0){
    this.total-=this.cart[idx].quantity*Math.floor(this.cart[idx].book.price.value);
    this.cart.splice(idx,1)
    }
